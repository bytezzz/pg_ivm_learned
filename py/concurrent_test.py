import time
from multiprocessing import Process, Barrier, Array, Value
...
def run_batch(result: Value):
    print("Start Running Batch")
    time_costs = Array("d", [0.0] * max_connection)

    processes = [
        Process(target=worker, args=(i, time_costs)) for i in range(max_connection)
    ]
